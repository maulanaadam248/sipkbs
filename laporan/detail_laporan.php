...
if (!function_exists('getKomoditasIcon')) {
    function getKomoditasIcon($nama_komoditas) {
        ...
        $icon = 'fa-seedling'; $color = '#16a34a'; $bg = 'rgba(22, 163, 74, 0.1)';
        if (strpos($k, 'kakao') !== false) { $icon = 'fa-leaf'; $color = '#78350f'; $bg = 'rgba(120, 53, 15, 0.1)'; } 
        elseif (strpos($k, 'kopi') !== false) { $icon = 'fa-egg'; $color = '#451a03'; $bg = 'rgba(69, 26, 3, 0.1)'; } 
        elseif (strpos($k, 'kelapa') !== false) { $icon = 'fa-seedling'; $color = '#047857'; $bg = 'rgba(4, 120, 87, 0.1)'; } 
        elseif (strpos($k, 'tembakau') !== false) { $icon = 'fa-leaf'; $color = '#65a30d'; $bg = 'rgba(101, 163, 13, 0.1)'; } 
        elseif (strpos($k, 'kapas') !== false) { $icon = 'fa-spa'; $color = '#0ea5e9'; $bg = 'rgba(14, 165, 233, 0.1)'; } 
        elseif (strpos($k, 'lada') !== false) { $icon = 'fa-leaf'; $color = '#064e3b'; $bg = 'rgba(6, 78, 59, 0.1)'; } 
        elseif (strpos($k, 'vanili') !== false) { $icon = 'fa-leaf'; $color = '#0d9488'; $bg = 'rgba(13, 148, 136, 0.1)'; } 
        elseif (strpos($k, 'nilam') !== false) { $icon = 'fa-leaf'; $color = '#15803d'; $bg = 'rgba(21, 128, 61, 0.1)'; } 
        elseif (strpos($k, 'wijen') !== false) { $icon = 'fa-seedling'; $color = '#d97706'; $bg = 'rgba(217, 119, 6, 0.1)'; } 
        elseif (strpos($k, 'rosella') !== false) { $icon = 'fa-spa'; $color = '#be123c'; $bg = 'rgba(190, 18, 60, 0.1)'; } 
        elseif (strpos($k, 'jarak') !== false) { $icon = 'fa-seedling'; $color = '#4d7c0f'; $bg = 'rgba(77, 124, 15, 0.1)'; }
...
// 2. Cek role - HANYA OPERATOR yang bisa akses halaman ini
if($_SESSION['role'] != 'operator') {
    header("Location: ../dashboard/dashboard.php");
    exit();
}

// 3. Pastikan ID Laporan tersedia di URL
if(!isset($_GET['id'])) {
    header("Location: manajemen_laporan.php");
    exit();
}

$laporan_id = $_GET['id'];
$balai_id_operator = $_SESSION['balai_id']; // Ambil ID Balai dari operator yang sedang login

// 4. QUERY KEAMANAN: Ambil data HANYA JIKA laporan tersebut milik balai operator ini
$query = "SELECT l.*, b.nama_balai 
          FROM laporan l 
          JOIN balai b ON l.balai_id = b.id_balai 
          WHERE l.id_laporan = ? AND l.balai_id = ?";
...
// Jika data tidak ditemukan (Mungkin laporan dihapus, atau operator mencoba mengintip laporan balai lain)
if(mysqli_num_rows($result) == 0) {
    $_SESSION['error'] = "Data tidak ditemukan atau Anda tidak memiliki akses ke laporan ini!";
    header("Location: manajemen_laporan.php");
    exit();
}
...
$page_title = "Detail Laporan - " . htmlspecialchars($_SESSION['nama_balai']);
$current_page = 'manajemen_laporan';
...
<style>
    ...
    .btn-edit-modern { background: linear-gradient(135deg, #10b981 0%, #059669 100%) !important; color: white !important; border: none !important; }
    ...
    <div class="container-fluid px-4 px-lg-5">
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4">
            <div class="mb-3 mb-md-0">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-1">
                        <li class="breadcrumb-item"><a href="manajemen_laporan.php" class="text-decoration-none text-muted">Laporan Balai Saya</a></li>
            ...
            <div class="d-flex gap-3 mb-4">
                <a href="riwayat_laporan.php" class="btn-modern-action btn-back-modern shadow-sm">
                    <i class="fas fa-arrow-left me-2"></i> Kembali
                </a>
                <a href="edit_laporan.php?id=<?= $laporan['id_laporan']; ?>" class="btn-modern-action btn-edit-modern shadow-sm">
                    <i class="fas fa-edit me-2"></i> Edit Data Saya
                </a>
            </div>
        </div>
        ...
        <div class="card border-0 shadow-sm rounded-4 bg-white overflow-hidden mb-5">
            ...
            <div class="card-body p-4 p-lg-5">
                ...
                <div class="row g-4">
                    <div class="col-md-6 col-lg-4">
                        <div class="info-box h-100">
                            <div class="detail-label"><i class="fas fa-university me-2"></i>Instansi Anda</div>
                    ...
                    <div class="col-md-6 col-lg-4">
                        <div class="info-box h-100 border-start border-primary border-4">
                            <div class="detail-label text-primary"><i class="fas fa-boxes me-2"></i>Jumlah Stok Anda</div>
                ...
                <div class="mt-5 pt-4 border-top text-center text-muted small">
                    <i class="fas fa-shield-alt me-1 text-success"></i> Anda hanya dapat melihat dan mengubah laporan milik balai Anda (<?= htmlspecialchars($_SESSION['nama_balai']); ?>).
                </div>
            </div>
        </div>
